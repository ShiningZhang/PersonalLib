...
{
	acl::acl_cpp_init();
	cluster_ = new acl::redis_client_cluster();
	// å½“æŸä¸ªè¿æ¥æ± ç»“ç‚¹å‡ºé—®é¢˜ï¼Œè®¾ç½®æ¢æµ‹è¯¥è¿æ¥ç»“ç‚¹æ˜¯å¦æ¢å¤çš„æ—¶é—´é—´éš”(ç§’)ï¼Œå½“è¯¥å€¼
	// ä¸º 0 æ—¶ï¼Œåˆ™ä¸æ£€æµ‹
	cluster_->set_retry_inter(retry_time);

	// è®¾ç½®é‡å®šå‘çš„æœ€å¤§é˜€å€¼ï¼Œè‹¥é‡å®šå‘æ¬¡æ•°è¶…è¿‡æ­¤é˜€å€¼åˆ™æŠ¥é”™
	cluster_->set_redirect_max(retry_max);

	// å½“é‡å®šå‘æ¬¡æ•° >= 2 æ—¶æ¯æ¬¡å†é‡å®šå‘æ­¤å‡½æ•°è®¾ç½®ä¼‘æ¯çš„æ—¶é—´(æ¯«ç§’)
	cluster_->set_redirect_sleep(redirect_sleep);

	cluster_->init(NULL, addrs, max_threads, conn_timeout, rw_timeout);
	...
	cmd_->set_cluster(cluster_, max_threads);
}
